...
*   ### Task ID: `CMS-IMP-001`
    ...
    *   #### Plan:
        1.  **Backend Tasks (FastAPI):**
            *   **Project Scaffolding:** Create a new directory `backend/cms/` to house all CMS-related backend modules, following a domain-driven structure (e.g., `backend/cms/articles`, `backend/cms/users`).
            *   **Data Modeling:** Define Pydantic models for `ArticleCreate`, `ArticleUpdate`, and `ArticleRead` based on the schema in `cms_research_v3.md`.
            *   **API Endpoints:** Implement the initial `/api/v1/articles` router with full asynchronous CRUD operations interacting with a dedicated `articles` collection in MongoDB.
            *   **Authentication:** Implement JWT-based authentication, preferably using the `fastapi-users` library. Create initial user roles ('Writer', 'Editor', 'Admin') and secure the article endpoints.
        2.  **Frontend Tasks (Next.js):**
            *   **Dependency Installation:** Install `@tiptap/react`, `@tiptap/starter-kit`, `react-hook-form`, `@hookform/resolvers`, `zod`.
            *   **Schema Definition:** Create `frontend/src/lib/zod/articleSchema.ts` for frontmatter validation, mirroring the backend models.
            *   **Component Development:** Build `FrontmatterForm.tsx`, `TiptapEditor.tsx` (with a basic schema), and assemble them into `ArticleEditor.tsx`.
            *   **Page Routing:** Create pages for the CMS dashboard (`/dashboard`), new article editor (`/editor/new`), and editing existing articles (`/editor/[id]`).
            *   **API Integration:** Connect frontend components to the backend API for data operations and implement login UI/state management for JWTs.
