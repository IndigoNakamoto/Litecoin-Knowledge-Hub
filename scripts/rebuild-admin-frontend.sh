#!/bin/bash
# Helper script to rebuild and restart only the admin frontend service
# This is useful during development when you only need to update admin frontend code
...
# Set production URLs (same defaults as run-prod.sh)
PROD_BACKEND_URL="${NEXT_PUBLIC_BACKEND_URL:-https://api.lite.space}"

echo "🔨 Rebuilding admin frontend service..."
echo "   NEXT_PUBLIC_BACKEND_URL=$PROD_BACKEND_URL"
echo ""

# Rebuild the admin frontend service
# Using --no-cache ensures a clean rebuild
# Pass build arg for NEXT_PUBLIC_BACKEND_URL (required for admin frontend build)
...
# Stop and remove the admin frontend container, then start it again
# This ensures the new image is used
$DOCKER_COMPOSE $COMPOSE_FILES up -d --force-recreate admin_frontend

echo ""
echo "✅ Admin frontend service rebuilt and restarted!"
echo ""
echo "📋 Checking admin frontend logs (Ctrl+C to exit)..."
echo ""

# Show logs (user can Ctrl+C to exit)
$DOCKER_COMPOSE $COMPOSE_FILES logs -f admin_frontend
